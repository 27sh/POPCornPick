...
<script>
    $(document).ready(function () {
        ...
        $(document).on('click', '.cinema-item', function () {
            ...
            var selectedCinema = cinemas.find(function (item) {
                return item.cinemaName === selectedCinemaName;
            });
            if (selectedCinema) {
                selectedCinemaNo = selectedCinema.cinemaNo;
            }
            ...
            $.ajax({
                ...
                success: function (schedule) {
                    ...
                    schedule.forEach(function (item) {
                        ...
                        totalSeats = item.room.roomType.roomTotalColumn * item.room.roomType.roomTotalRow;
                        
                        var dub = item.movieShowDetail.dubbing;
                        var format = "2D"; // Assuming format is 2D, modify as needed if format information is available
                    ...
                    Object.keys(movieSchedules).forEach(function (title) {
                        ...
                        movieSchedules[title].schedules.forEach(function (scheduleItem) {
                            ...
                            var scheSeats = $('<li>').addClass('sche_seats')
                                .append($('<span style="float: left; font-weight: 700;">').text(scheduleItem.format))
                                .append(' ')
                                .append($('<span style="float: right;color: green;font-weight: 500;">').text(scheduleItem.dubbing ? '더빙' : '자막'));
        ...
        $('#cinemaLocation').change(function () {
            ...
            // 첫 번째 옵션의 cinemaNo를 전역 변수에 저장
            cinemaName.prop('selectedIndex', 0).trigger('change');
        });

        // cinemaName 선택 시 selectedCinemaNo 업데이트
        $('#cinemaName').change(function () {
            var selectedCinemaName = $(this).val();
